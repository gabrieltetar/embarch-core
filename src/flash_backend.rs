...
fn locate(tool: &str) -> Option<PathBuf> {
    env_override(tool)
        ...
        .or_else(|| extra_candidates(tool).into_iter().find(|p| p.is_file()))
}
...
pub fn run(
    ...
    let mut cmd = match backend {
        ...
        Backend::JLink { exe } => {
            ...
            let script = jlink_script(firmware_path, erase);
...
fn jlink_script(firmware_path: &Path, erase: bool) -> String {
    let mut s = String::from("si SWD\nspeed 4000\nr\n");
    if erase {
        s.push_str("erase\n");
    }
    // `loadfile` picks the format from the extension and honours the hex's own
    // addresses, which is what makes this equivalent to `west flash`.
