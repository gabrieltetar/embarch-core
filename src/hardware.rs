...
fn parse_format(format: &str) -> Result<Format> {
    match format.to_lowercase().as_str() {
        ...
        "bin" => Ok(Format::Bin(Default::default())),
        "hex" => Ok(Format::Hex),
        "uf2" => Ok(Format::Uf2),
...
/// "esp32c3"). `firmware_path` is a path Core can read locally — the caller is
/// responsible for getting the file onto whatever machine Core runs on.
pub fn flash(chip: &str, firmware_path: &Path, format: &str) -> Result<()> {
